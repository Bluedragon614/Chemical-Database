...
public class ChemicalDatabase {
    ...
    public boolean load(String elementsFile, String binaryCompoundsFile, String reactionsFile, int firstValence, int increment, boolean periodicTrendsChanged) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, DatabaseGUI.WARNING + DatabaseGUI.ELEMENTS_FILE + DatabaseGUI.WARNING_NOT_FOUND);
            noElements = true;
        }catch(NumberFormatException nfx) {
            JOptionPane.showMessageDialog(null, DatabaseGUI.ERROR + DatabaseGUI.ERROR_BOTCHED_FILE + DatabaseGUI.ELEMENTS_FILE);
        }
        
        //initialize periodic table
        this.periodicTable = new PeriodicTable(firstValence, increment, this.sortElements());
        
        //if element file errors, dont load other 2 files
        if (!noElements) {
            ...
            try{      
                ...
                JOptionPane.showMessageDialog(null, DatabaseGUI.WARNING + DatabaseGUI.BINARY_COMPOUNDS_FILE + DatabaseGUI.WARNING_NOT_FOUND);
            }catch(NullPointerException npx){
                JOptionPane.showMessageDialog(null, DatabaseGUI.ERROR + DatabaseGUI.ERROR_FILE_MISMATCH_1 + DatabaseGUI.BINARY_COMPOUNDS_FILE + DatabaseGUI.ERROR_FILE_MISMATCH_2 + DatabaseGUI.ELEMENTS_FILE_NAME);
            }catch(NumberFormatException nfx) {
                JOptionPane.showMessageDialog(null, DatabaseGUI.ERROR + DatabaseGUI.ERROR_BOTCHED_FILE + DatabaseGUI.ELEMENTS_FILE);
            }
            ...
            try{
                ...
                if (periodicTrendsChanged) {
                    while((line = in.readLine()) != null){
                        ...
                        if (newReaction.getID() != Reaction.IMPOSSIBLE_REACTION_ID && newReaction.getID() != Reaction.REPEATED_REACTION_ID) {
                            newReaction.getProduct().setCommonName(in.readLine());   
                        } else {
                            this.removeReaction(newReaction);
                            in.readLine();
                        }
                    }
                } else {
                ...
                JOptionPane.showMessageDialog(null, DatabaseGUI.WARNING + DatabaseGUI.REACTIONS_FILE + DatabaseGUI.WARNING_NOT_FOUND);
            } catch(NullPointerException npx){
                JOptionPane.showMessageDialog(null, DatabaseGUI.ERROR + DatabaseGUI.ERROR_FILE_MISMATCH_1 + DatabaseGUI.REACTIONS_FILE + DatabaseGUI.ERROR_FILE_MISMATCH_2 + DatabaseGUI.ELEMENTS_FILE_NAME);
            } catch(NumberFormatException nfx) {
                JOptionPane.showMessageDialog(null, DatabaseGUI.ERROR + DatabaseGUI.ERROR_BOTCHED_FILE + DatabaseGUI.ELEMENTS_FILE);
            }
        }
    ...
    public Reaction simulateReaction(Element reactant1, Element reactant2) {
        ...
        //if reaction is invalid, return respective ID identifier
        if (newReaction.getID() != Reaction.IMPOSSIBLE_REACTION_ID) {
            if (!isNewReaction(newReaction))
                newReaction.setID(Reaction.REPEATED_REACTION_ID);

            if (newReaction.getID() != Reaction.REPEATED_REACTION_ID) {
                this.reactionManager.addReaction(newReaction);
    ...
    public boolean editElements(Element elementToBeEdited, String commonName, double stpDensity, double bp, double mp, String colour, String molecularFormula, int numProton, double electronegativity, int numNeutron, double molarMass, double ionizationEnergy){
        ...
        //Temporary variables to store element data before delete
    ...
    public boolean editBinaryCompounds(BinaryCompound binaryCompoundToBeEdited, String commonName, double stpDensity, double bp, double mp, String colour){
        //remove binary compound to allow for repeat checks
        this.chemicals.remove(binaryCompoundToBeEdited);
        binaryCompoundToBeEdited.getComponent1().getElement().getCompoundsInvolved().remove(binaryCompoundToBeEdited);
        binaryCompoundToBeEdited.getComponent2().getElement().getCompoundsInvolved().remove(binaryCompoundToBeEdited);
        ...
        binaryCompoundToBeEdited.setReactionsInvolved(binaryCompoundToBeEdited.getReactionsInvolved());
    ...
    public boolean removeChemical(Chemical chemicalToBeRemoved) {
        ...
        //if the chemical is an element, remove all compounds involved
        ...
        //if the chemical is a compound remove all pointers from component elements
        if (chemicalToBeRemoved instanceof BinaryCompound)
            ((BinaryCompound)chemicalToBeRemoved).removePointers();
        
        //no matter if it is element or compound, remove reactions related to chemical
    ...
    public boolean removeReaction(Reaction reactionToBeRemoved){
        if(reactionToBeRemoved != null){
            ...
            //However since our current reaction algorithm only allows for one unique reaction for each product
    ...
    //Searching Methods (sequential & binary)
    ...
    public Element searchElementByAtomicSymbol(String symbol){
        ...
        if(symbol.equals(Element.DEFAULT_ATOMIC_SYMBOL))
            return PeriodicTable.UNKNOWN_ELEMENT;
        return null;
    }
        ...
        if(!chemicals.isEmpty()){
            ...
            for(int i = 0; i < elements.get(elements.size()-1).getNumProton(); i++){
                if(elements.get(i).getNumProton() != i + 1)
                    elements.add(i, PeriodicTable.UNKNOWN_ELEMENT);
            }
        }
    ...
    //checking for repeats of important properties
    ...
    //Methods to return a randomized permutation of the characters of a chemical name
    //wrapper function + randomization
